...
@interface FuWuOrderInfoViewController ()<UITextViewDelegate>
@property(nonatomic,strong)FuWuOrderInfoBlock block;
...
@implementation FuWuOrderInfoViewController
...
-(void)newAdDetail{
    ...
    [_mainScrollView addSubview:CDImage];
    ...
    [_mainScrollView addSubview:nameCell];
    ...
    [_mainScrollView addSubview:AdCell];
    
    _mainScrollView.contentSize=CGSizeMake(self.view.width, AdCell.bottom+10*self.scale);
    [self newFuWUDetail];
}
-(void)newFuWUDetail{
    CellView *PresonCell=[[CellView alloc]initWithFrame:CGRectMake(0, _mainScrollView.contentSize.height, self.view.width, 44*self.scale)];
    ...
    [_mainScrollView addSubview:PresonCell];
    NSArray *IArr=[_infoDic objectForKey:@"products"];
    NSString *Items=@"";
    for (NSDictionary *dic in IArr) {
    ...
    NSArray *Arr=@[@"服务项目：",@"下单时间：",@"配送时间：",@"备      注："];
      NSArray *ValArr=@[Items,[NSString stringWithFormat:@"%@",_infoDic[@"order_create_time"]],[NSString stringWithFormat:@"%@",_infoDic[@"send_time"]],[NSString stringWithFormat:@"%@",_infoDic[@"memo"]]];
    float SetY=PresonCell.bottom;
    for(int i=0;i<Arr.count;i++){
        ...
        cell.bottomline.hidden=i!=Arr.count-1;
    ...
    CellView *OrderCell=[[CellView alloc]initWithFrame:CGRectMake(0, SetY+10*self.scale, self.view.width, 40*self.scale)];
  ...
  //  OrderCell.bottomline.frame=CGRectMake(10*self.scale, OrderCell.height-.5, self.view.width-20*self.scale, .5);
    [_mainScrollView addSubview:OrderCell];
    Arr=@[@"订单编号：",@"下单时间：",@"完成时间：",@"配送员：",@"配送电话："];
  ValArr=@[[NSString stringWithFormat:@"%@",_infoDic[@"sub_order_no"]],[NSString stringWithFormat:@"%@",_infoDic[@"order_create_time"]],[NSString stringWithFormat:@"%@",_infoDic[@"finish_time"]],[NSString stringWithFormat:@"%@",_infoDic[@"delivery_starff_name"]],[NSString stringWithFormat:@"%@",_infoDic[@"delivery_starff_mobile"]]];
    SetY=OrderCell.bottom;
    for(int i=0;i<Arr.count;i++){
        CellView *cell=[[CellView alloc]initWithFrame:CGRectMake(0, SetY, self.view.width, 25*self.scale)];
        cell.titleLabel.text=Arr[i];
        cell.titleLabel.textColor=grayTextColor;
...
-(void)BottomView{
    ...
    NSString *Model=[NSString stringWithFormat:@"%@",[_infoDic objectForKey:@"delivery_mode"]];
    ...
    switch (state) {
        case 2:
        {
            _mainScrollView.frame=CGRectMake(0, self.NavImg.bottom, self.view.width, self.view.height-self.NavImg.bottom-40*self.scale);
...
-(void) orderReceiving:(UIButton *)button WithTextView:(UITextView *) Text andDeliveryID:(NSString*) deliveryid{
   // NSDictionary *dic =[_dataSource objectAtIndex:button.tag];
    [self.activityVC startAnimating];
    AnalyzeObject *analy=[[AnalyzeObject alloc]init];
    [analy AcceptOrderWithUser_ID:[Stockpile sharedStockpile].ID Order_no:[NSString stringWithFormat:@"%@",[_infoDic  objectForKey:@"sub_order_no"]] msg:[Text.text trimString] DeliveryID:deliveryid Block:^(id models, NSString *code, NSString *msg) {
...
//        [self.activityVC startAnimating];
//        AnalyzeObject *analy=[[AnalyzeObject alloc]init];
//        [analy AcceptOrderWithUser_ID:[Stockpile sharedStockpile].ID Order_no:[NSString stringWithFormat:@"%@",[_infoDic  objectForKey:@"sub_order_no"]] msg:[Text.text trimString] Block:^(id models, NSString *code, NSString *msg) {
//            
//            NSMutableDictionary *model=[[Stockpile sharedStockpile].model mutableCopy];
//            NSMutableDictionary *Info=[[model objectForKey:@"shop_info"] mutableCopy];
//            [Info setObject:[[NSString stringWithFormat:@"%@",[models objectForKey:@"msg"]] EmptyStringByWhitespace] forKey:@"order_msg"];
...
//        }];
  ...
  /*  [self.activityVC startAnimating];
    AnalyzeObject *analy=[[AnalyzeObject alloc]init];
    [analy AcceptOrderWithUser_ID:[Stockpile sharedStockpile].ID Order_no:[NSString stringWithFormat:@"%@",[_infoDic  objectForKey:@"sub_order_no"]] Block:^(id models, NSString *code, NSString *msg) {
        [self.activityVC stopAnimating];
        [self ShowAlertWithMessage:msg];
        if ([code isEqualToString:@"0"]) {
            [self PopVC:nil];
