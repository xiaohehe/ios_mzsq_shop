//
//  ShenQingShangPinLeiShangJiaViewController.m
//  MuZhiSheQu
//
...
@interface ShenQingShangPinLeiShangJiaViewController ()<UITextFieldDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,VPImageCropperDelegate>
...
@property(nonatomic,strong)NSDictionary *Sheng,*Shi,*Xian,*Location;
@end

@implementation ShenQingShangPinLeiShangJiaViewController
...
-(void)newView{
    ...
    float SetY =0;
    NSArray *Arr=@[@"联系人",@"电话",@"店铺名称",@"选择类型",@"选择店铺所属区域",@"店铺详细地址",@"店铺坐标",@"身份证上传",@"营业执照",@"资质证书"];
...
-(void)SelectedTap:(UITapGestureRecognizer *)tap{
    ...
    [_linMenuVC SelectedShopType:@"1" Block:^(NSDictionary *Type, NSDictionary *Detail) {
      ...
      /*  GetBaiDuMapViewController *huoQuVC=[[GetBaiDuMapViewController alloc]init];
        [huoQuVC getZuoBiaoBlock:^(NSDictionary *dic) {
            UITextField *NameText=(UITextField *)[self.view viewWithTag:15];
            _TypeDic=[dic mutableCopy];
            NameText.text=[NSString stringWithFormat:@"%@,%@",[dic objectForKey:@"latitude"],[dic objectForKey:@"longitude"]];
        }];
        ...
        [_menuVC SelectedNum:3 Block:^(NSDictionary *Sheng, NSDictionary *Shi, NSDictionary *Xian, NSDictionary *Zhen) {
            cell.contentLabel.text=[NSString stringWithFormat:@"%@",[Zhen objectForKey:@"name"]];
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 0) {
        ...
        imagePickerController.allowsEditing = actionSheet.tag == 1;
...
- (void)imagePickerController: (UIImagePickerController *)picker didFinishPickingMediaWithInfo: (NSDictionary *)info{
    ...
    if (_TapImg==1) {
        ...
        UIImageView *Img=(UIImageView *)[self.view viewWithTag:37];
        Img.image=newImg;
        _YY=base64img;
    }else if (_TapImg == 4){
        UIImageView *Img=(UIImageView *)[self.view viewWithTag:38];
        Img.image=newImg;
        _ZZ=base64img;
    }
...
-(void)QueRenTiJiao:(id)sender{
    ...
    UITextField *AdressText=(UITextField *)[self.view viewWithTag:14];
    NSString *adress = [AdressText.text trimString];
    if ([adress isEmptyString]) {
        [self ShowAlertWithMessage:@"请输入您的店铺地址"];
        return;
    }
    ...
    NSString *Str=@"";
//    for (NSString *key in [_TypeDic allKeys]) {
//        NSDictionary *item=[_TypeDic objectForKey:key];
//        Str=[NSString stringWithFormat:@"%@,%@",Str,[item objectForKey:@"id"]];
//    }
//    if (Str.length>0) {
//        Str=[Str substringFromIndex:1];
//    }
 
   /* if(!_ComID || _ComID.length<1){
        [self ShowAlertWithMessage:@"请选择认证的小区"];
        return;
    }*/
    ...
    UITextView *DESText=(UITextView *)[self.view viewWithTag:20];
    ...
    [analy ApplyServeShopWithUser_ID:[Stockpile sharedStockpile].ID Contact_name:name Mobile:tel Shop_name:shopName Address:adress Serve_type:Str Community:_ComID Idcard1:_Card1 Idcard2:_Card2 License:_YY Certification:_ZZ Buss_scope:des Shop_type:@"1" Is_weishop:_IsWeiShang lng:[NSString stringWithFormat:@"%@",[_Location objectForKey:@"longitude"]] lat:[NSString stringWithFormat:@"%@",[_Location objectForKey:@"latitude"]] province:[NSString stringWithFormat:@"%@",[_Sheng objectForKey:@"id"]] city:[NSString stringWithFormat:@"%@",[_Shi objectForKey:@"id"]] district:[NSString stringWithFormat:@"%@",[_Xian objectForKey:@"id"]] Block:^(id models, NSString *code, NSString *msg) {
