...
@property(nonatomic,strong)ShiBlock block;
...
-(void)ReshData{
    ...
    [analt GetCityListWithProvince_id:_PID Block:^(id models, NSString *code, NSString *msg) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
  ...
  NSDictionary *dic = _dataSource[indexPath.section][indexPath.row];
    [_xianVC ReshViewByCID:[NSString stringWithFormat:@"%@",dic[@"id"]] Name:[NSString stringWithFormat:@"%@",dic[@"name"]] Block:^(NSDictionary *Xian, NSDictionary *Zhen) {
        _block(dic,Xian,Zhen);
        self.view.frame = CGRectMake(self.view.width, 0, self.view.width , self.view.height);
    }];
    _xianVC.isAdress=_isAdress;
