...
@interface UnOnSellGoodsViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,GoodsEditTableViewCellDelegate>
...
@implementation UnOnSellGoodsViewController
...
-(void)ReshData{
    ...
    [analy ProdListWithUser_ID:[Stockpile sharedStockpile].ID Class_id:@"" Status:@"2" Keyword:_Key Pindex:[NSNumber numberWithInteger:_index] Block:^(id models, NSString *code, NSString *msg) {
...
-(void)EditButtonEvent:(NSIndexPath *)sender{
    ...
    AddGoodsVc.unit=[[NSString stringWithFormat:@"%@",[dic objectForKey:@"unit"]] EmptyStringByWhitespace];
...
-(void)serachVi{
    ...
    UITapGestureRecognizer*searchGesture = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(search)];
    [IconImage addGestureRecognizer:searchGesture];
    [SearchBG addSubview:IconImage];
    self.searchText=[[UITextField alloc]initWithFrame:CGRectMake(8*self.scale, 0, SearchBG.width-IconImage.height-5*self.scale , SearchBG.height)];
    self.searchText.font=DefaultFont(self.scale);
    self.searchText.placeholder=@"请输入店名/商品";
...
-(void)bottomVi{
    ...
    [xiaJiaBtn setTitle:@"上架" forState:UIControlStateNormal];
...
-(void)ShangJiaButtonEvent:(id)sender{
    ...
    [self.activityVC startAnimating];
    AnalyzeObject *analy=[[AnalyzeObject alloc]init];
    [analy OnOffShelveProdWithUser_ID:[Stockpile sharedStockpile].ID Prod_id:IDS State:@"1" Block:^(id models, NSString *code, NSString *msg) {
        [self.activityVC stopAnimating];
        [self ShowAlertWithMessage:msg];
        if ([code isEqualToString:@"0"]) {
            _index=0;
