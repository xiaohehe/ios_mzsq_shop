//
//  LeiBieGoodsListViewController.m
//  MuZhiSheQu
//
...
@interface LeiBieGoodsListViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,GoodsEditTableViewCellDelegate>
...
@implementation LeiBieGoodsListViewController
...
-(void)ReshData{
    ...
    [analy ProdListWithUser_ID:[Stockpile sharedStockpile].ID Class_id:_ID Status:@"1" Keyword:_Key Pindex:[NSNumber numberWithInteger:_index] Block:^(id models, NSString *code, NSString *msg) {
...
#pragma mark - 导航
-(void)newNav{
    self.TitleLabel.text=[NSString stringWithFormat:@"%@类商品列表",_Name];
...
-(void)saveBtnEvent:(UIButton *)sender{
    ...
    AddGoodsVc.class_id=_ID;
    AddGoodsVc.class_Name=_Name;
    [self.navigationController pushViewController:AddGoodsVc animated:YES];
}
